...
public class PlantController {
	...
	@RequestMapping(path = "createPlant.do", method = RequestMethod.POST)
	public String createdPlant(Plant plant, Model model, HttpServletRequest request, HttpSession session, RedirectAttributes redir) {
		if (session.getAttribute("loggedInUser") == null) {
			return "createPlant";
		} else {
			...
			redir.addAttribute("plantId", newPlant.getId());
			User user = new User();
			if (session.getAttribute("loggedInUser") != null) {
				user = userDao.findById(((User) session.getAttribute("loggedInUser")).getId());
			}
	...
	public String createRedirect(@RequestParam int userId, int plantId, Model model) {
